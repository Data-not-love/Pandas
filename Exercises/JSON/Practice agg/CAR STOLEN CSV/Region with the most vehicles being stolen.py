...
result  = merge_2_data_frame.groupby(["region","location_id"]).agg(
    number_of_stolen_vehicles = pd.NamedAgg(column="vehicle_type",aggfunc="count")
)
sorted_result = result.sort_values(by="number_of_stolen_vehicles", ascending=False)
print(sorted_result)